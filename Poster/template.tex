...
\begin{block}{Introduction}
	Modelling disease spread is crucial for governments to develop effective contingency plans for large-scale pandemics. The UK's "inadequate response" to COVID-19 serves as evidence for poor governmental action \cite{Frowde2020FailPrepare}. Motivated by the need for better predictive tools, we attempted to model disease spread within Greater Manchester using a reaction-diffusion equation, chosen because the random large-scale motion of people is analogous to Brownian diffusion.
