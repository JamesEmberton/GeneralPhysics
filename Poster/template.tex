...
\usepackage[
	...
	size=a1, % Paper size
	scale=1.25, % Scale, it's important to adjust this so your content fits nicely in the template
...
\begin{block}{Results: Simulation vs Data}
	Using the parameters of transmissibility taking into account weather conditions $\beta_0 =0.48 $ \cite{2021} and using the spatial density given for each county of Greater Manchester from the March 2021 Census data [reference database here]. Comparing the two diagrams of the simulated spread over many days and the actual spread can see the pattern closely aligns in the earlier days of the initial days while the graphs diverage at the later times with the actual distribution of infected people being less uniform and scattered both in the centre and edges whereas the model shows a wave like distribution in which only infection left on the outer edges of Greater Manchester. 
