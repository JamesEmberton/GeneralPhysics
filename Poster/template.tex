...
\begin{block}{Results: Simulation vs Data}
	Using the parameters of transmissibility taking into account weather conditions $\beta_0 =0.48 $ \cite{TALIB2021} and using the spatial density given for each county of Greater Manchester from the March 2021 Census data [reference database here]. Comparing the two diagrams of the simulated spread over many days and the actual spread can see the pattern closely aligns in the earlier days of the initial days while the graphs diverge at the later times with the actual distribution of infected people being less uniform and scattered both in the centre and edges whereas the model shows a wave like distribution in which only infection left on the outer edges of Greater Manchester. 
	Put things here - actual distributed to be added.
\end{block}

\begin{block}{Results: Simulation vs Data}
	Using the parameters of transmissibility taking into account weather conditions $\beta_0 =0.48 $ \cite{TALIB2021} and using the spatial density given for each county of Greater Manchester from the March 2021 Census data \cite{CENSUS2021}. Comparing the two diagrams of the simulated spread over many days and the actual spread can see the pattern closely aligns in the earlier days of the initial days while the graphs diverge at the later times with the actual distribution of infected people being less uniform and scattered both in the centre and edges whereas the model shows a wave like distribution in which only infection left on the outer edges of Greater Manchester. 
	Put things here - actual distributed to be added.
\end{block}
...
<<<<<<< HEAD
\begin{block}{Results: Simulation vs Data}
	Using the parameters of transmissibility taking into account weather conditions $\beta_0 =0.48 per day$ \cite{TALIB2021} and diffusion coefficient of $D = 0.85 m^2 s^{-1}$. 
	Furthermore, the mean recovery time was assumed to be $\tau = 7 days$ \cite{}note (nhs website). COVID-19 infection was modelled 
	using the spatial density given for each county of Greater Manchester from the March 2021 Census data [reference database here].
	
	Put things here - actual distributed to be added. 
\end{block}

=======
>>>>>>> b973e94c79a3c26730a1471ee36b7599a1312c60
%------------------------------------------------
...
<<<<<<< HEAD
		...
		\caption{COVID-19 infection over time in Greater Manchester.
		Using parameters of intial transmission rate $\beta_0 = 0.48 per day$ and diffusion coefficient of $D = 0.85 m^2 s^{-1}$ 
	...
	\caption{}
=======
		\includegraphics[width=\linewidth]{Infection_density_figure2.png}
...
\begin{block}{Results: Spread of Covid From Data}
	\begin{figure}
		...
		\caption{Disease spread in Boston over time.}
	\end{figure}
\end{block}

\begin{block}{Discussion}
We found that though our model was successful at modelling the initial spread of the Covid 19 pandemic. In particular, the exponential growth seen in FigXYZ matches near perfectly data seen in \cite{FRY2021104400}. However, after this, the model starts to diverge from real life data. This is likely because we made the assumption that people’s behaviour wouldn’t change when infected. In reality, as time went on, and more was understood about Covid, people started to self isolate when they first experienced symptoms, and the first lockdowns started to occur in March 2020 \cite{iog}. However, this doesn’t make the idea of using diffusion-reaction useless or invalid in all cases. Instead, it implies that the best use of such a model is to predict what would happen should the spread of the virus not be contained. 
...
\begin{block}{Conclusions} 
	In conclusion, while the diffusion-reaction equation was able to more accurately model the beginning of a disease outbreak the model starts to diverge at later times. This is primarily due to the simplifying assumptions that were made for our model. A better model would take into account the individual motion of people using a stochastic model such as a Monte-Carlo method to more accurately reflect this random walk.
\end{block}
%----------------------------------------------------------------------------------------
%	REFERENCES
%----------------------------------------------------------------------------------------
